...
import { TrendingUp, MessageCircle, Phone, DollarSign, Eye, CheckCircle, XCircle, Target, Zap, BarChart3, MousePointer, Users } from "lucide-react";
...
interface VeiculoPipeline {
  ...
  fipe?: number;
  // Metricas de anuncio
  turbinamento: number;    // R$ gasto em impulsionamento
  impressoes: number;      // Quantas pessoas viram o anuncio
  cliques: number;         // Quantos clicaram
  contatos: number;        // Quantas pessoas chamaram
  diasNoAr: number;
  fotos: boolean;
  leadsReais: Lead[];
}
...
function calcProb(leads: number, preco: number, fipe?: number): number {
  ...
  if (fipe) {
    const disc = (fipe - preco) / fipe;
    if (disc > 0.05) r *= 2;
    else if (disc > 0.02) r *= 1.5;
    else if (disc < 0) r *= 0.7;
  }
  return Math.min(r, 1);
}

// ─── DADOS INICIAIS (com metricas de anuncio) ──────────────
    ...
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <p className="text-slate-400 text-sm mt-1">
          {veiculos.length} veiculos | R$ {totalTurbinamento.toLocaleString("pt-BR")} investidos | {totalImpressoes.toLocaleString("pt-BR")} impressoes | {totalCliques} cliques | {totalContatos} contatos | {totalLeads} leads ativos
        </p>
      </motion.div>

      {/* CARDS DE VEICULOS COM METRICAS */}
          ...
          return (
            ...
            >
              ...
              <div className="flex gap-3 text-[11px] text-slate-500 mb-2">
                <span>CTR {ctr}%</span>
                <span>CPC R${cpc}</span>
                <span>{v.diasNoAr}d no ar</span>
              </div>

              {v.fipe && (() => {
                const d = ((v.fipe - v.preco) / v.fipe * 100);
                return (
                  <p className={`text-xs mt-2 ${d > 0 ? "text-green-400" : d < 0 ? "text-red-400" : "text-yellow-400"}`}>
      ...
      {/* PIPELINE DO VEICULO SELECIONADO */}
      {veiculo && (
        <GlowCard>
          <div className="flex justify-between items-center mb-4">
            <div>
              ...
              <p className="text-xs text-slate-400">
                R$ {veiculo.turbinamento} investidos | {veiculo.impressoes.toLocaleString("pt-BR")} imp. | CTR {(veiculo.impressoes > 0 ? (veiculo.cliques / veiculo.impressoes * 100) : 0).toFixed(1)}% | {veiculo.contatos} contatos
              </p>
            </div>
            <button
            ...
            <div className="bg-slate-800 rounded-lg p-4 mb-4 space-y-3">
              ...
              <div className="flex gap-3">
                <select
                  ...
                  onChange={e => setNewLead(p => ({ ...p, origem: e.target.value as typeof ORIGENS[number] }))}
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
            {ESTAGIOS.map(estagio => {
              ...
              return (
                <div
                  key={estagio.key}
                  className={`rounded-lg p-3 ${estagio.bg} border border-slate-700/30 min-h-[120px]`}
                >
                  <div className={`flex items-center gap-1.5 mb-2 ${estagio.color}`}>
                    <Icon className="w-4 h-4" />
                    <span className="text-xs font-semibold">{estagio.label}</span>
                    ...
                    <div key={lead.id} className="bg-slate-800/80 rounded-lg p-2 text-xs mb-1.5">
                      ...
                      <div className="flex gap-1 mt-2 flex-wrap">
                        {ESTAGIOS.filter(e => e.key !== estagio.key).map(e => (
                          <button
                            key={e.key}
                            onClick={() => moverEstagio(veiculo.id, lead.id, e.key)}
                            className={`text-[10px] px-2 py-0.5 rounded ${e.bg} ${e.color} hover:opacity-80`}
                          >
                            {e.label}
                          </button>
                        ))}
